...
| ランク | スコア区間 | レーティング値 |
|---|---|---|
| SSS+ | 1,010,000（理論値） | 譜面定数 + 2.2 |
| SSS+ | 1,010,000 〜 1,009,000 | 10点ごとに +0.0005（加算） |
| SSS+ | 1,009,000 | 譜面定数 + 2.15 |
| SSS | 1,009,000 〜 1,007,500 | 10点ごとに +0.001 |
| SSS | 1,007,500 | 譜面定数 + 2.0 |
| SS+ | 1,007,500 〜 1,005,000 | 10点ごとに +0.002 |
| SS+ | 1,005,000 | 譜面定数 + 1.5 |
| SS | 1,005,000 〜 1,000,000 | 10点ごとに +0.001 |
...
- MAS・ULT それぞれで、スコアが **理論値（1,010,000点）** の楽曲数をカウントする
  ※ EXP・ADV・BAS は理論値枠の集計対象外とする
- 換算値（/1000）= 合計理論値数 ÷ 1000（chuni-forceの合計値に加算される）
- maxでの換算値 = 全MAS・ULT楽曲数の合計 ÷ 1000（全曲理論値を取った場合の換算値）

**理論値枠テーブルの表示項目：**

| カラム | 内容 |
|---|---|
| MASTER | MASTERで理論値を取得している楽曲数 |
| ULTIMA | ULTIMAで理論値を取得している楽曲数 |
| SUM | MASTER + ULTIMA の合計 |
| 換算値（/1000） | SUM ÷ 1000 |
