...
```
chuni-force = (ベスト枠50曲の単曲chuni-force値の平均)
              + (理論値枠上位50曲の単曲FORCE値の平均)
              + (MAS & ULT の理論値総数 ÷ 10000)
```

ユーザーの実際のスコアに基づく値と、全曲理論値（1,010,000点）を仮定した場合の最大値（chuni-force理論値）の両方を算出する。
...
3. 対象となった各楽曲について、計算式 `単曲FORCE値 = (Const ÷ 15.0)^2 * 2` で**単曲FORCE値**を算出する。
    > ※ この式により、Const 15.0 の楽曲で単曲FORCE値が丁度 `2.000` となり、50曲平均時も総合値に対し `+2.000` の加算となる。
4. **最終的な理論値枠（理論値枠 average）** = 対象50曲の単曲FORCE値の **平均値**（CHUNIFORCEの総合値に加算される）
...
| 単曲FORCE値 | 上記の累乗計算によって算出された単曲FORCE値 |

---

## 5. 表示要件

### 5.1 chuni-force 総合値・内訳の表示
...
| chuni-force 理論値 | 全曲理論値（1,010,000点、かつMAS/ULT全曲制覇）を仮定した場合の最大chuni-force値（小数点第3位まで） |
| ベスト枠 average | ベスト枠50曲の単曲chuni-force値の平均値（小数点第4位まで） |
| 理論値枠 average | 理論値枠（AJC Bonus）50曲の単曲FORCE値の平均値（小数点第4位まで） |
...
chuni-force の値に応じて **CLASS 1〜10** のエンブレム情報が割り当てられる。
